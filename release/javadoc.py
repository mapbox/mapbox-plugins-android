...
def generate_javadoc(version_name, plugin):
    ...
    update_current_version(file_path='../plugin-%s/gradle.properties' % plugin, file_var=constants.GRADLE_TOKEN, version=version_name)
    subprocess.Popen(['make', 'javadoc-%s' % plugin], cwd=constants.PLUGIN_ROOT_PATH).wait()
    subprocess.Popen(['mv', 'release', version_name], cwd='../plugin-%s/build/docs/javadoc/' % plugin).wait()
    ...
    subprocess.Popen(['mv', version_name, '../../../../../android-docs/api/plugins/%s' % plugin], cwd='../plugin-%s/build/docs/javadoc/' % plugin).wait()
    ...
    subprocess.Popen(['git', 'commit', '-m', COMMIT_MESSAGE], cwd=constants.ANDROID_DOCS_ROOT_PATH).wait()
    subprocess.Popen(['git', 'push', '-u', 'origin', BRANCH_NAME], cwd=constants.ANDROID_DOCS_ROOT_PATH).wait()
